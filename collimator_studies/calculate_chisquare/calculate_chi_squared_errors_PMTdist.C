...
void calculate_chi_squared_errors_PMTdist(char* filename1=NULL, char* filename2=NULL, char* col_idx =NULL) {
    ...
    float abs = 1.30;
    float refl = 0.90;
    // Grab collimator positions from file
    float colX = 3314.91;
    float colY=0;
    float colZ=-3352.01;
    t1->SetBranchAddress("injectorVtxX",&colX);
    t1->SetBranchAddress("injectorVtxY",&colY);
    t1->SetBranchAddress("injectorVtxZ",&colZ);
    t1->GetEntry(0);
    ...
    size_t found=f1.find_last_of('/');
    cout << " folder: " << f1.substr(0,found) << endl;
    string filepath = f1.substr(0,found);
    string outputFile = Form("%s/chisquare_collimator_%d.root",filepath.c_str(),colIdx);

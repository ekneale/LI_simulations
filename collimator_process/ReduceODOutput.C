...
// WCSim Includes
#include "/home/kneale/hk-software/wcsim_od_calib/include/WCSimRootEvent.hh"
#include "/home/kneale/hk-software/wcsim_od_calib/include/WCSimRootGeom.hh"
...
void ReduceODOutput( const char *inFileName = "wcsim.root", const char *outFileName = "reduced_output.root", bool verbosity = 0){ 
  ...
  outBranch->Branch("trueTime",&trueTime,"trueTime/F");//true time of Cherenkov photon at PMT
  outBranch->Branch("recTime",&recTime,"recTime/F");//digitised hit time
  outBranch->Branch("trueNumPhotons",&trueNumPhotons,"trueNumPhotons/F");
  outBranch->Branch("recCharge",&recCharge,"recCharge/F");// digitised charge
  ...
  for (int ev = 0; ev < nEvent; ev++){ // Loop over events
    if (ev % 1000 == 0) std::cerr << "Event # " << ev << std::endl; 
